...
export default function ChatBox(props) {
  ...
  const calcMinutesAgo = (m: Message) =>
    Math.abs(Math.round((Date.now() - m.time.getTime()) / 60000)).toString() +
    " mins ago";
    ...
    for await (const message of messages) {
      if (message.type === "text") {
        newMessages.push({
          ...
          time: new Date(
            message.createdOn.getYear(),
            message.createdOn.getMonth(),
            message.createdOn.getDay(),
            message.createdOn.getHours(),
            message.createdOn.getMinutes(),
            message.createdOn.getSeconds(),
            0
          ),
      ...
      <Panel color="bg-bdg" height="h-100" className="rounded-7">
        ...
        <div className="h10">
          <MessageInput
            ...
            className="form-input fixed-bottom"
          />
        </div>
      </Panel>
    </>
