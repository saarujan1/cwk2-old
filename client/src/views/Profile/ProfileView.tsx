...
export default function ProfileView() {
  ...
  const listHobbies = userData.user.hobbies.map((hobby) => (
    <button className="c-btn-purple">{hobby}</button>
  ));

  console.log(JSON.stringify(userData));
  return (
    <>
      <h2 className="text-light-cream">
        Welcome,{" "}
        {userData.user.id ? (
          <span className="text-darker-gray fw-bold">{userData.user.id}</span>
        ) : (
          <p>No Name</p>
        )}
      </h2>
